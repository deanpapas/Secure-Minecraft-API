...
public class RemoteSession {
	...
	protected void handleLine(String line) throws Exception {
		...
		try {
			...
			// MAC verification
			if (calculatedMac.equals(macString)) { // MAC verification successful
				...
				System.out.println("==============");
				handleCommand(methodName, args); // handle command

			} else { // MAC verification failed
				System.out.println("MAC verification failed!");
				handleCommand("testFailCommand", null);
			}
		...
		try {
			...
			switch (c) {
				...
				case "world.getHeight":
					send(world.getHighestBlockYAt(parseRelativeBlockLocation(args[0], "0", args[1]))
							- origin.getBlockY());
		...
		for (int y = minY; y <= maxY; ++y) {
			for (int x = minX; x <= maxX; ++x) {
				for (int z = minZ; z <= maxZ; ++z) {
					...
					// adds data
					if (withData)
						blockData.append(currBlock.getData()).append(";");
				}
			}
		}
	...
	private String getHeights(Location pos1, Location pos2) {
		...
		return heightData.substring(0, heightData.length() > 0 ? heightData.length() - 1 : 0); // We don't want last
																								// comma
	}
	...
	private Location parseLocation(World world, double x, double y, double z, double originX, double originY,
			double originZ) {
